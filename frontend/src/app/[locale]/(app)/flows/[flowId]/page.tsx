              ...
              {runs.slice(0, 50).map((r) => (
                ...
                >
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    {r.outcome && (
                      <OutcomeBadge outcome={r.outcome} label={tMode(`outcome_${r.outcome}`)} />
                    )}
                    <Badge variant="outline">
                      {tFlows(`trigger.${r.trigger_kind}`)}
                    </Badge>
                    {r.probe_response_status !== null &&
                      r.probe_response_status !== undefined && (
                        <Badge variant="outline">
                          {tMode("probeStatus", { status: r.probe_response_status })}
                        </Badge>
                      )}
                    {r.probe_duration_ms !== null &&
                      r.probe_duration_ms !== undefined && (
                        <span className="text-[10px] sh-muted">
                          {tMode("probeDuration", { ms: r.probe_duration_ms })}
                        </span>
                      )}
                  ...
                  {r.probe_output_excerpt && !r.output_summary && (
                    <pre className="mt-1 max-h-32 overflow-auto whitespace-pre-wrap break-words rounded-sm bg-black/5 p-2 font-mono text-[11px] dark:bg-white/5">
                      {r.probe_output_excerpt.slice(0, 1000)}
                    </pre>
                  )}
                  {r.error && (
...
function OutcomeBadge({
  ...
  const map: Record<FlowRunOutcome, "default" | "warning" | "success" | "danger" | "primary"> = {
    pending: "default",
    success: "success",
    silent_2xx: "success",
    nonempty_output: "primary",
    escalated_to_agent: "primary",
    http_error: "danger",
    script_error: "danger",
    timeout: "warning",
    ssrf_blocked: "danger",
  ...
  return <Badge variant={map[outcome] ?? "default"}>{label}</Badge>;
}
