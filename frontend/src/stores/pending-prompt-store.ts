...
export interface PendingPromptPayload {
  ...
  webSearch?: boolean;
  /** Composer mode (Plan / Thinking / Flash / Subagent) carried over so the
   *  first turn fires with the user's choice. */
  mode?: ChatMode;
  /** ``provider:model`` selector picked in the composer. Forwarded to the
   *  WS transport on the first turn so the new session honours the user's
   *  choice without an extra round trip. ``null`` means "use the user's
   *  saved preference / agent default". */
  model?: string | null;
}
