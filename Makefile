...
## ─── Hybrid dev (db+redis in docker, backend+frontend on host) ─
# Use when you need direct LAN access by IP, or want to bypass Docker
# Desktop's port-proxy when it gets flaky.
# Prereqs:
#   - backend/.venv is created on the host
#   - frontend/node_modules is populated (pnpm install)
# The repo-root .env already ships with host-friendly defaults
# (DB_HOST=localhost, REDIS_HOST=localhost, STORAGE_LOCAL_PATH=./.data/storage);
# docker compose silently overrides these to container hostnames inside
# containers via its `environment:` blocks.

.PHONY: infra-up
infra-up: ## Start only db + redis (for host-run backend/frontend)
...
.PHONY: backend-host
backend-host: ## Run backend on host (bind 0.0.0.0:8000)
	cd backend && .venv/Scripts/python -m cli.commands server run --host 0.0.0.0 --port 8000 --reload

.PHONY: frontend-host
frontend-host: ## Run frontend on host (bind 0.0.0.0:3000)
	cd frontend && node_modules/.bin/next dev --port 3000 --hostname 0.0.0.0

.PHONY: dev-host
dev-host: ## One-shot: infra-up + spawn backend/frontend in new windows (Windows)
	$(COMPOSE_DEV) up -d db redis
	cmd //c start "senharness-backend" cmd /k "cd backend && .venv\Scripts\python -m cli.commands server run --host 0.0.0.0 --port 8000 --reload"
	cmd //c start "senharness-frontend" cmd /k "cd frontend && node_modules\.bin\next dev --port 3000 --hostname 0.0.0.0"
