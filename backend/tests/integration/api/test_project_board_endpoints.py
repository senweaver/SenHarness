...
async def test_get_board_returns_kanban_snapshot(async_client):
    headers, _ = await _bootstrap(async_client)
    r = await async_client.post("/api/v1/boards", headers=headers, json={"name": "Board"})
    board_id = r.json()["id"]
    await async_client.post(
...
async def test_patch_board_renames(async_client):
    headers, _ = await _bootstrap(async_client)
    r = await async_client.post("/api/v1/boards", headers=headers, json={"name": "Old"})
    board_id = r.json()["id"]
...
async def test_archive_board_soft_deletes(async_client):
    headers, _ = await _bootstrap(async_client)
    r = await async_client.post("/api/v1/boards", headers=headers, json={"name": "Doomed"})
    board_id = r.json()["id"]
    r = await async_client.post(f"/api/v1/boards/{board_id}/archive", headers=headers)
    assert r.status_code == 200, r.text
...
async def test_create_card_member_path(async_client):
    headers, ws_id = await _bootstrap(async_client)
    r = await async_client.post("/api/v1/boards", headers=headers, json={"name": "B"})
    board_id = r.json()["id"]
...
async def test_complete_and_archive_card(async_client):
    ...
    r = await async_client.post(f"/api/v1/cards/{card_id}/complete", headers=headers)
    assert r.status_code == 200
    body = r.json()
    assert body["column"] == "done"
    assert body["completed_at"] is not None

    r = await async_client.post(f"/api/v1/cards/{card_id}/archive", headers=headers)
    assert r.status_code == 200
...
async def test_cross_workspace_isolation(async_client):
    ...
    r = await async_client.post("/api/v1/boards", headers=headers_a, json={"name": "Tenant-A"})
    board_id = r.json()["id"]

    headers_b, _ = await _bootstrap(async_client)
    r = await async_client.get(f"/api/v1/boards/{board_id}", headers=headers_b)
    assert r.status_code == 404
    assert _err_code(r.json()) == "board.not_found"
...
async def test_list_cards_for_agent_filters_done(async_client):
    ...
    r = await async_client.get(f"/api/v1/agents/{agent_id}/cards", headers=headers)
    assert r.status_code == 200, r.text
    rows = r.json()
