...
def register(ctx) -> None:
    async def _on_pre_tool(*, tool_name: str, **_kwargs: object) -> None:
        print(f"[echo_logger] pre_tool_call: {tool_name}")

    async def _on_post_tool(*, tool_name: str, ok: bool = True, **_kwargs: object) -> None:
        status = "ok" if ok else "error"
        print(f"[echo_logger] post_tool_call: {tool_name} -> {status}")
