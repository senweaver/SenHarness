...
from app.services.mcp_client import McpClientError, McpSdkUnavailable

log = logging.getLogger(__name__)


async def _absorb_oauth_config(
    db: DBSession,
    *,
    ...
    """Translate the form-side OAuth payload into ``auth_json``.

    When the operator types a plaintext ``client_secret`` we rotate
    it into the workspace vault and rewrite ``auth_json`` to point
    at the freshly-minted reference. The plaintext never round-trips
    back through the API response.
    """
    if oauth is None:
        return auth_json
    out: dict[str, Any] = dict(auth_json)
    out["type"] = "oauth"
    out["client_id"] = oauth.client_id
    out["token_url"] = oauth.token_url
    out["scopes"] = oauth.scopes
    out["refresh_grace_seconds"] = oauth.refresh_grace_seconds
    if oauth.client_secret_ref:
        out["client_secret_ref"] = oauth.client_secret_ref
    ...
    if oauth.client_secret:
        ...
        if existing is not None:
            ...
            await vault_svc.create_secret(
                ...
                name=secret_name,
                plaintext=oauth.client_secret,
                kind=VaultItemKind.OAUTH,
                metadata={"purpose": "mcp_oauth_client_secret", "server_slug": server_slug},
            )
...
async def create_server(
    ...
    auth_json = await _absorb_oauth_config(
        db,
        workspace_id=ws_id,
        server_slug=body.slug,
        auth_json=body.auth_json,
        oauth=body.auth_oauth,
    )
...
async def update_server(
    ...
    auth_oauth = patch.pop("auth_oauth", None)
    if auth_oauth is not None:
        slug = patch.get("slug") or row.slug
        merged_auth = patch.get("auth_json") or dict(row.auth_json or {})
        oauth_obj = McpOAuthConfig.model_validate(auth_oauth)
        patch["auth_json"] = await _absorb_oauth_config(
            db,
            workspace_id=ws_id,
            server_slug=slug,
            auth_json=merged_auth,
            oauth=oauth_obj,
        )
    if patch:
...
class _McpServiceUnavailable(AppError):
    code = "mcp.service_unavailable"
    default_status = status.HTTP_503_SERVICE_UNAVAILABLE


class _McpClientFailed(AppError):
    code = "mcp.client_failed"
...
async def list_server_tools(
    ...
    """Probe the MCP server and return its current tool catalogue.

    Connects via the M2.5.4 client (resolves OAuth, runs the SDK
    handshake, drops the connection on the way out) so the workspace
    UI can verify that the operator's transport / OAuth combo
    actually works without staging a chat turn first.
    """
    ws_id = _require_workspace(workspace_id)
    await _require_admin(db, workspace_id=ws_id, identity_id=identity_id)
    ...
    except McpSdkUnavailable as e:
        raise _McpServiceUnavailable(str(e), code="mcp.sdk_unavailable") from e
    except McpClientError as e:
        raise _McpClientFailed(
            str(e),
            code=getattr(e, "code", "mcp.client_failed"),
        ) from e
    await db.commit()
    return [
        {
