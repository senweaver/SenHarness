...
from app.schemas.auth import LoginIn, RefreshOut, RegisterIn, TokenOut
...
async def register(body: RegisterIn, db: DBSession) -> IdentityRead:
    identity = await svc.register(db, email=body.email, name=body.name, password=body.password)
    await db.commit()
