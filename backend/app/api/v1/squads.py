...
@router.post("/{squad_id}/star", response_model=StarSquadOut)
async def star_squad(
    ...
    await ws_svc.ensure_member_access(db, workspace_id=ws_id, identity_id=identity_id)
    await svc.get_or_404(db, squad_id, workspace_id=ws_id)
    starred, pinned_state = await svc.star_squad(
        db,
    ...
    await db.commit()
    return StarSquadOut(squad_id=squad_id, starred=starred, pinned=pinned_state)


@router.delete("/{squad_id}/star", status_code=status.HTTP_204_NO_CONTENT)
async def unstar_squad(
    ...
    await ws_svc.ensure_member_access(db, workspace_id=ws_id, identity_id=identity_id)
    await svc.get_or_404(db, squad_id, workspace_id=ws_id)
    await svc.unstar_squad(db, identity_id=identity_id, squad_id=squad_id)
    await db.commit()
