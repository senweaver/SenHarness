...
from app.core.errors import EmailNotVerified, PermissionDenied, Unauthorized
...
async def require_active_identity(
    db: Annotated[AsyncSession, Depends(db_session)],
    identity_id: Annotated[uuid.UUID, Depends(current_identity_id)],
) -> Identity:
    """Ensure the caller is an ACTIVE identity (not PENDING email verification).

    Whitelist endpoints (``/me``, ``/auth/verify-email/*``,
    ``/auth/resend-verification``, ``/auth/logout``) deliberately use
    :data:`CurrentIdentityId` instead so a PENDING user can still land
    on them and finish onboarding.
    """
    ...
    if identity.status == IdentityStatus.PENDING:
        raise EmailNotVerified("email_unverified", code="auth.email_unverified")
    return identity


RequireActiveIdentity = Annotated[Identity, Depends(require_active_identity)]
