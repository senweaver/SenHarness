...
class Workspace(UuidPkMixin, TimestampMixin, SoftDeleteMixin, Base):
    ...
    # Marks soft-deleted workspaces whose slug must not be reused. Set
    # by ``DELETE /workspaces/{id}`` (M0.12) so a malicious user can't
    # create + delete + re-create the same slug repeatedly to bypass
    # the per-identity quota or to squat on a recognisable slug after
    # an organization moves on. Personal-workspace slug allocation and
    # manual ``POST /workspaces`` both consult ``slug_tombstoned``
    # before accepting a candidate.
    slug_tombstoned: Mapped[bool] = mapped_column(
