...
class McpTransport(StrEnum):
    """Transport channels supported by the M2.5.4 client.

    ``stdio`` spawns a local process and talks JSON-RPC over its
    pipes; ``sse`` opens a one-way Server-Sent Events stream paired
    with a POST endpoint; ``streamable_http`` is the modern bidi
    HTTP-with-streaming transport that replaces SSE in newer MCP
    deployments. The legacy ``http`` value is preserved for
    backward-compat health-ping rows but is normalised to
    ``streamable_http`` at client build time.
    """
...
class McpServer(UuidPkMixin, TimestampMixin, SoftDeleteMixin, WorkspaceScopedMixin, Base):
    ...
    transport: Mapped[str] = mapped_column(
        String(40), default="stdio", nullable=False, server_default="stdio"
    )
    # Legacy column — pre-M2.5.4 servers stored their HTTP base in
    # ``endpoint``. New SSE / streamable_http rows write to ``url`` instead;
    # ``endpoint`` stays for the M0 health-ping path so older bookmarks keep
    # working. ``transport=stdio`` ignores both.
