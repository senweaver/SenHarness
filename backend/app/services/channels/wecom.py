...
class WeComProvider(ChannelProvider):
    ...
    def metadata(cls) -> ChannelProviderMeta:
        return ChannelProviderMeta(
            ...
            description=(
                "WeChat Work self-built application. Uses SHA1 message "
                "signatures + AES-CBC payload encryption on inbound "
                "pushes, and the standard message/send REST API for "
                "outbound replies. The VerifyURL handshake is handled "
                "automatically."
            ),
