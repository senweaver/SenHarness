...
class DiscordProvider(ChannelProvider):
    ...
    def metadata(cls):
        ...
        return ChannelProviderMeta(
            ...
            description=(
                "Discord application / bot. Webhook mode validates the "
                "Ed25519 signature on every interaction; stream mode "
                "(via discord.py Gateway) lets the bot listen on "
                "messages without a public Interactions URL."
            ),
            ...
            # Stream mode dials the Gateway with just the bot token —
            # the Ed25519 ``public_key`` is only needed when Discord's
            # signed Interactions endpoint posts back to us in webhook
            # mode.
            mode_required_fields={
                "stream": ("bot_token",),
                "webhook": ("bot_token", "public_key"),
            },
            mode_optional_fields={
                "stream": ("allowed_guild_ids", "allow_dms"),
                "webhook": ("application_id", "allowed_guild_ids", "allow_dms"),
            },
        )
    ...
    async def run_stream(
        ...
        *,
        ...
        from app.services.channels._discord_stream import run_gateway_stream

        await run_gateway_stream(channel=channel, dispatch=dispatch, stop=stop)
