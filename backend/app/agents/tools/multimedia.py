...
Resolution of credentials follows the same path as the main LLM kernel:
the first enabled OpenAI-compatible provider in the workspace wins; no
hard-coded env var.
"""
...
async def _resolve_openai() -> _OpenAIish | None:
    ...
    Falls back to the ``OPENAI_API_KEY`` / ``OPENAI_BASE_URL`` env vars so the
    tools are usable before a workspace has a proper Vault-backed provider.
    """
    ctx = get_context()
    ...
    async with factory() as session:
        stmt = (
            ...
            .where(
                ...
                # OpenAI-compatible providers. We accept anything behind the
                # /v1/images and /v1/audio endpoints — in practice that's
                # OpenAI itself, OpenRouter, Azure OpenAI, and self-hosted
                # vLLM/SGLang builds that proxy the endpoints.
                ModelProvider.kind.in_(
                    [
                        ProviderKind.OPENAI,
...
def _from_env() -> _OpenAIish | None:
    ...
    return _OpenAIish(
        api_key=key,
        base_url=os.environ.get("OPENAI_BASE_URL", _DEFAULT_OPENAI_BASE).rstrip("/"),
    )
